...
/**
 * Map implementation for the Leaflet mapping service.
 *
 * @author j.bartels
 * @package TYPO3
...
class Map extends \JBartels\WecMap\MapService\Map {
	...
	public  $markerClassName = 'JBartels\\WecMap\\MapService\\Leaflet\\Marker';
	...
	/**
	 * Class constructor for Leaflet Map
	 */
	...
	/**
	 * Enables controls for OSM Maps, for example zoom level slider or mini
...
//				break;
	...
	public function drawMap() {
		...
		if ($hasThingsToDisplay && $hasHeightWidth && $browserKey ) {
			...
			$wmcssURL = $siteRelPath . 'Resources/Public/CSS/wecmap.css';
			$lcssURL = $siteRelPath . 'Resources/Public/JavaScript/leaflet/leaflet.css';
			$ljsURL  = $siteRelPath . 'Resources/Public/JavaScript/leaflet/leaflet.js';
			$ljsURL  = $siteRelPath . 'Resources/Public/JavaScript/leaflet/leaflet-src.js';
			$mmURL   = $siteRelPath . 'Resources/Public/JavaScript/leaflet.markercluster/leaflet.markercluster-src.js';
			$mmCSSURL = $siteRelPath . 'Resources/Public/JavaScript/leaflet.markercluster/MarkerCluster.css';
			$mmCSSDefURL = $siteRelPath . 'Resources/Public/JavaScript/leaflet.markercluster/MarkerCluster.Default.css';
			$ovURL   = $siteRelPath . 'Resources/Public/JavaScript/leaflet.omnivore/leaflet-omnivore.min.js';
			...
			if (TYPO3_MODE == 'FE') {
				$GLOBALS['TSFE']->additionalHeaderData['wec_map_leaflet'] = '<link href="'.$wmcssURL.'" rel="stylesheet"></link>'
				                                                          . '<link href="'.$lcssURL.'" rel="stylesheet"></link>'
				                                                          . '<script src="'.$ljsURL .'" type="text/javascript"></script>'
																		  . '<link href="'.$mmCSSURL.'" rel="stylesheet"></link>'
																		  . '<link href="'.$mmCSSDefURL.'" rel="stylesheet"></link>'
				                                                          . '<script src="'.$mmURL .'" type="text/javascript"></script>'
				                                                          . '<script src="'.$ovURL .'" type="text/javascript"></script>'
				                                                          ;
				...
				$htmlContent .= '<link href="'  . \TYPO3\CMS\Core\Utility\GeneralUtility::getIndpEnv('TYPO3_SITE_URL') . $wmcssURL . '" rel="stylesheet"></link>';
				$htmlContent .= '<link href="'  . \TYPO3\CMS\Core\Utility\GeneralUtility::getIndpEnv('TYPO3_SITE_URL') . $lcssURL . '" rel="stylesheet"></link>';
				$htmlContent .= '<script src="' . \TYPO3\CMS\Core\Utility\GeneralUtility::getIndpEnv('TYPO3_SITE_URL') . $ljsURL . '" type="text/javascript"></script>';
				$htmlContent .= '<link href="'  . \TYPO3\CMS\Core\Utility\GeneralUtility::getIndpEnv('TYPO3_SITE_URL') . $mmCSSURL . '" rel="stylesheet"></link>';
				$htmlContent .= '<link href="'  . \TYPO3\CMS\Core\Utility\GeneralUtility::getIndpEnv('TYPO3_SITE_URL') . $mmCSSDefURL . '" rel="stylesheet"></link>';
				$htmlContent .= '<script src="' . \TYPO3\CMS\Core\Utility\GeneralUtility::getIndpEnv('TYPO3_SITE_URL') . $mmURL . '" type="text/javascript"></script>';
				$htmlContent .= '<script src="' . \TYPO3\CMS\Core\Utility\GeneralUtility::getIndpEnv('TYPO3_SITE_URL') . $ovURL . '" type="text/javascript"></script>';
...
//				$htmlContent .= '<script src="' . \TYPO3\CMS\Core\Utility\GeneralUtility::getIndpEnv('TYPO3_SITE_URL') . $omURL . '" type="text/javascript"></script>';
	...
	protected function generateStaticMap($markers, $center = true, $alt = '') {
		...
		$maptype = 'mapnik';
		if($center) {
			$url = 'https://staticmap.openstreetmap.de/staticmap.php?center='.floatval($this->lat) .','.floatval($this->long) .'&zoom='.intval($this->zoom).'&size='.intval($this->width).'x'.intval($this->height).'&maptype='.urlencode($maptype).'&markers='.urlencode($markers);
		} else {
			$url = 'https://staticmap.openstreetmap.de/staticmap.php?size='.intval($this->width).'x'.intval($this->height).'&maptype='.urlencode($maptype).'&markers='.urlencode($markers);
		}
...
WecMap.createMap("'. $this->mapName . '", "leaflet" );';
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/wec_map/map_service/leaflet/class.tx_wecmap_map_leaflet.php'])	{
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/wec_map/map_service/leaflet/class.tx_wecmap_map_leaflet.php']);
}
